...
def build_chemistry_stone_sets(dataset, input_word2idx=None):
    ...
    io_id = vocab.get('<io>', 22)
    
    # Build sets of special and potion token IDs for stone parsing
    special_ids = set()
    for tok in ['<pad>', '<sos>', '<eos>', '<io>', '<item_sep>', '<unk>']:
        if tok in vocab:
            special_ids.add(vocab[tok])
    potion_ids = set()
    ...
    for key in sorted_support_keys:
        ...
        chemistry_stones[key] = extract_stones_from_support(
            list(key), io_id, item_sep_id, potion_ids, special_ids
        )
        
    return stone_sets, sorted_support_keys, chemistry_stones

def find_all_disjoint_pairs(dataset, input_word2idx=None):
    import itertools
    stone_sets, sorted_support_keys, chemistry_stones = build_chemistry_stone_sets(dataset, input_word2idx)
    ...
    for i, j in itertools.combinations(range(len(sorted_support_keys)), 2):
        ...
        # Require zero stone overlap between the two chemistries' support stones.
        # Since each unique stone maps to exactly one target_class_id,
        # this also guarantees disjoint target_class_ids.
        if chemistry_stones[sk1] & chemistry_stones[sk2] == frozenset():
...
def build_chemistry_identity_pairs(dataset, chemistry_id_1, chemistry_id_2, input_word2idx=None, require_disjoint=True):
    ...
    if require_disjoint:
        assert chemistry_stones[sk1] & chemistry_stones[sk2] == frozenset(), \
            "Chemistries have overlapping support stones!"
...
def build_all_disjoint_pairs(dataset, input_word2idx=None, max_pairs=None):
    ...
    all_pairs = []
    print("Building disjoint pairs")
    for i, j in tqdm(disjoint_pairs_idx):
...
def run_full_sweep(model, all_pairs, input_word2idx, setup='noising', patch_mlp=False, batch_size=128):
    ...
    for comp in tqdm(components, desc="Sweeping components"):
        ...
        for i in tqdm(range(0, len(all_pairs), batch_size), desc=f"Batches for {comp}", leave=False):
            ...
            for pair in batch:
                ...
                support, _ = split_support_query(c_in_flat.tolist(), item_sep_id)
                mask = torch.zeros(len(c_in_flat), dtype=torch.bool)
                mask[:len(support)] = True
                masks.append(mask)
    ...
    # Print results table sorted by mean LSE score descending
    sorted_comps = sorted(components, key=lambda c: results[c]["lse_mean"], reverse=True)
...
def main():
    ...
    if args.calibration_exp:
        ...
        # Run full sweep
        all_pairs = build_all_disjoint_pairs(dataset, input_word2idx, max_pairs=args.max_pairs)
