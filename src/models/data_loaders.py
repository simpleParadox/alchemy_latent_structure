...
class AlchemyDataset(Dataset):
    ...
    def _normalize_reward_in_state(self, state_str: str, canonical_reward: str = '-3') -> str:
        """
        Normalize reward feature in a stone state string to a canonical value.
        
        Args:
            state_str: Stone state string like '{color: red, size: small, roundness: medium_round, reward: -3}'
            canonical_reward: The canonical reward value to use (default: '-3')
        
        Returns:
            Normalized state string with reward replaced
        """
        # Replace any reward value with the canonical one
        # Pattern matches: reward: followed by optional whitespace and any of the reward values
        normalized = re.sub(r'reward:\s*(-3|-1|1|3)', f'reward: {canonical_reward}', state_str)
        return normalized
    ...
    def _build_separate_vocabularies(self, json_file_path: str):
        ...
        if self.output_format == "stone_states":
            self.output_word2idx, self.output_idx2word = self._build_stone_state_vocab(json_file_path) # using the same_reward_value will reduce the number of unique stone states if the reward features are merged.
...
def collate_fn(batch: List[Dict[str, torch.Tensor]], pad_token_id: int, sos_token_id: int, eos_token_id: int, task_type: str = "seq2seq", model_architecture: str = "encoder", prediction_type: str = None,
               dataset=None, max_seq_len=2048, truncate=False,
               padding_side='right') -> Dict[str, torch.Tensor]:
    """
