...
if __name__ == "__main__":
    ...
    argument_parser.add_argument("--eval_episodes", type=int, help="Number of evaluation episodes", default=5)
    ...
    episode_num = 0
    unique_chemistries_found = 0
    target_unique_chemistries = 5  # We want to find 5 unique chemistries
    
    graph_data = {}
    while episode_num < MAX_EPISODES and unique_chemistries_found < target_unique_chemistries:
    ...
    print(f"\nFound {unique_chemistries_found} unique chemistries out of {episode_num} episodes processed")
    env.close()
    ...
    for chem, items in eval_chems:
        ...
        is_complete_graph, is_newly_seen_unique_chemistry = check_chemistry_uniqueness_and_completeness(chem, episode_num)
        if not is_complete_graph:
            print(f"Skipping incomplete graph in episode {episode_num}")
            episode_num += 1
            continue
        ...
        current_chemistry_graph = {}
        try:
            nodes = chem.graph.node_list.nodes
            edges = chem.graph.edge_list.edges
            # Import necessary types locally if not already imported at top level
            from dm_alchemy.types.stones_and_potions import LatentStone, AlignedStone, PerceivedStone, unalign
            ...
            for current_node in nodes:
                ...
                try:
                    ...
                    # 2. Convert LatentStone -> AlignedStone using stone_map
                    aligned_stone = chem.stone_map.apply_inverse(latent_stone)
                    # We need the reward for the PerceivedStone, use the node's reward
                    aligned_stone_with_reward = AlignedStone(aligned_stone.reward, aligned_stone.aligned_coords)

                    # 3. Convert AlignedStone -> PerceivedStone using rotation
                    perceived_stone = unalign(aligned_stone_with_reward, chem.rotation)
