...
class SymbolicAlchemy(dm_env.Environment, abc.ABC):
  ...
  def __init__(
    ...
    self._num_stones_per_trial = None
    # Find out the number of stones per trial from items_gen if possible
    try:
      trial_items = items_gen(0)
      self._max_stones = len(trial_items.stones)
      self._num_stones_per_trial = self._max_stones
    except Exception:
      self._max_stones = MAX_STONES
      self._num_stones_per_trial = MAX_STONES
  ...
  def observation_spec(self):
    num_stone_features, num_potion_features = slot_based_num_features(
        self.observe_used)
    obs_features = ((num_stone_features * self._max_stones) +
  ...
  def action_spec(self):
    ...
    num_actions = self._max_stones * (MAX_POTIONS + 1) + num_special_actions
  ...
  def observation(self):
    ...
    stone_features = np.concatenate(
        [default_stone_features for _ in range(self._max_stones)], axis=0)
    ...
    for stone in existing_stones:
      ...
      assert 0 <= stone_ind < self._max_stones, 'stone idx out of range'
...
def slot_based_stone_feature_dims(observe_used, rotated_stone_positions, max_stones=MAX_STONES):
...
def slot_based_stone_reward_dims(observe_used, max_stones=MAX_STONES):
...
def get_symbolic_alchemy_level(
  ...
  def chemistry_gen():
    # The arguments to the Chemistry constructor are the potion map, stone map,
    # graph and rotation. These are partial functions.
    return Chemistry(
